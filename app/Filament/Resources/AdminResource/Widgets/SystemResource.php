...
class SystemResource extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        $monitoring = Monitoring::monitor(
            config("monitoring.monitoring.interface")
        );
        ...
        $memory_usg_percent = Number::format(
            ($memory_usg / $resource["total-memory"]) * 100,
            2
        );
        return [
            Stat::make("CPU Usage", "$cpu_usage%")
                ->description(
                    "{$resource["cpu"]} (x{$resource["cpu-count"]}) {$resource["cpu-frequency"]} MHz"
                )
