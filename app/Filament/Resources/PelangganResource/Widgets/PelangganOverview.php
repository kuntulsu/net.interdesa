...
class PelangganOverview extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        $tagihan = Tagihan::where(
            "tipe_tagihan",
            TipeTagihanEnum::BULANAN,
        )->first();
        $pembayaran = $tagihan?->pembayaran;
        $progress_tagihan_percent =
            $pembayaran?->count() <= 0 || $pelanggan_count <= 0
                ? 0
                : ($pembayaran?->count() / $pelanggan_count) * 100;
        $total_tagihan_terbayar =
            "IDR " .
            Number::format(
                $tagihan?->pembayaran()?->sum("nominal_tagihan") ?? 0,
                2,
            );
        ...
        return [
            Stat::make("Total Pelanggan", $pelanggan_count)
                ...
                ->color("success"),
            Stat::make(
                "Persentase Pelanggan Membayar",
                number_format($progress_tagihan_percent, 2) . " %",
